...
void RFont::OnUpdate() {
	if(_width != EngineInst->screen_width() || _height != EngineInst->screen_height()) {
		...
		//_frameSurface = SDL_CreateRGBSurface(0,_width,_height,32,0,0,0,0);
	...
	//printText(100, 100, textColor, 0, "Hello test text!!!");
...
void RFont::printTextRAW(TTF_Font *font, SDL_Surface* surface, int x, int y, SDL_Color colorText, SDL_Color colorBorder, int border, int alignment, float scale, const char *text)
{
	...
	else
	{
		...
		/*if(scale != 1.0f) {
			textSurface->w = int (scale * textSurface->w);
			textSurface->h = int (scale * textSurface->h);
		}*/
		...
		//SDL_BlitScaled(  _frameSurface, NULL, _frameSurface, &stretchRect );
